(function () {
  ...
  var _SETTING_CATEGORIES = getSettingCategories;
  var _SETTING_DESCS = getSettingDescs;
  var _ENV_DESCS = getEnvDescs;
  var _DB_TABLES = getDbTables;
  ...
  function showPanel() {
    ...
    $$('[data-action]').forEach(function (btn) {
      ...
      btn.addEventListener('click', function () {
        doPowerAction(btn.dataset.action);
      });
    });
  ...
  function switchTab(tab) {
    ...
    switch (tab) {
      case 'dashboard':
        loadDashboard();
        S.pollTimers.push(setInterval(loadDashboard, 30000));
        break;
      case 'map':
        initMap();
        loadMapData();
        S.pollTimers.push(setInterval(loadMapData, 15000));
        break;
      case 'players':
        loadPlayers();
        break;
      ...
      case 'chat':
        loadChat();
        S.pollTimers.push(setInterval(loadChat, 8000));
        break;
      case 'settings':
  ...
  function renderSchedule(container, sched, _context) {
    ...
    for (var i = 0; i < sched.todaySchedule.length; i++) {
      ...
      if (ps && Object.keys(ps).length > 0 && typeof tippy !== 'undefined') {
        tippy(div, {
          content: buildScheduleTip(fullDisplayName, colorCls, ps),
          allowHTML: true,
          theme: 'translucent',
          placement: 'bottom',
          popperOptions: {
    ...
    for (var i = 0; i < _schedEdit.profiles.length; i++) {
      (function (idx) {
        ...
        dupeBtn.onclick = function () {
          ...
          var suffix = 2;
          while (_schedEdit.profiles.indexOf(newName) >= 0) {
            newName = name + '-copy' + suffix++;
          }
  ...
  if (_lmBtn)
    _lmBtn.addEventListener('click', function () {
      window.__loadMoreActivity && window.__loadMoreActivity();
    });
  ...
  function showItemPopup(slot) {
    ...
    if (isTrackedInstance && contextSteamId) {
      var holder = S.players.find(function (p) {
        return p.steamId === contextSteamId;
      });
    ...
    try {
      ...
      if (movements.length > 0) {
        ...
        if (movements.length > 5) {
          ...
          html +=
            '<span class="text-accent cursor-pointer hover:underline" onclick="if(S.tier>=3){switchTab(\'items\');}">';
