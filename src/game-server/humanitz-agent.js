...
function discoverPlayerSaveDir(savePath, explicitDir) {
  ...
  const candidates = [_path.join(saveDir, derived), _path.join(saveDir, 'DedicatedSaveMP')];
  const seen = new Set();
...
function _playerFingerprint(file) {
  return [file.relPath, String(file.mtimeMs), String(file.size), String(AGENT_VERSION_VALUE), PARSER_SIGNATURE].join(
    '|',
  );
}
...
function buildPlayersFromPlayerFiles(savePath, outputPath, explicitPlayerDir, mainPlayers) {
  ...
  const previousManifest =
    previousCache && previousCache.playerManifest && previousCache.playerManifest.files
...
function parseAndWrite(savePath, outputPath, pretty, idMapPath, playerDirPath) {
  ...
  const playerCacheInfo = playerCache.playerCacheStats
    ? ', discovered ' +
      playerCache.playerCacheStats.discovered +
      ' player files, parsed ' +
      playerCache.playerCacheStats.parsed +
      ', reused ' +
      playerCache.playerCacheStats.reused +
      ', removed ' +
      playerCache.playerCacheStats.removed +
      ', errors ' +
      playerCache.playerCacheStats.errors
    : '';
  console.log(
    '[Agent] Parsed ' +
      playerCount +
