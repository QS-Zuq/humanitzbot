...
describe('_fetchThreadMessages', () => {
  ...
  function mockThread(messages: Array<{ id: string; content: string; embeds: unknown[] }>) {
    // Discord.js messages.fetch returns a Collection (Map-like, ordered by ID descending)
    return {
      messages: {
        async fetch({ limit, before }: { limit?: number; before?: string } = {}) {
  ...
  function msg(
    id: number,
    { embedTitle, embedDesc, content }: { embedTitle?: string; embedDesc?: string; content?: string } = {},
  ) {
...
describe('_findMatchingThreads', () => {
  function mockChannel(
    activeThreads: Array<{ id: string; name: string }>,
    archivedThreads: Array<{ id: string; name: string }>,
  ) {
    return {
      threads: {
        async fetchActive() {
