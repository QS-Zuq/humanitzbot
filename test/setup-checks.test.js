...
describe('setup-checks', () => {
  describe('checkPrerequisites', () => {
    ...
    it('reports missing RCON keys as warnings', () => {
      ...
      assert.ok(warnings.some((w) => w.key === 'RCON_HOST'));
      assert.ok(warnings.some((w) => w.key === 'RCON_PASSWORD'));
    });
    ...
    it('skips SFTP when skipSftp is true', () => {
      ...
      assert.strictEqual(issues.filter((i) => i.key.startsWith('SFTP') || i.key.startsWith('FTP')).length, 0);
    });

    it('detects placeholder values', () => {
      process.env.DISCORD_TOKEN = 'your_token_here';
      const issues = checkPrerequisites();
      assert.ok(issues.some((i) => i.key === 'DISCORD_TOKEN' && i.type === 'missing'));
    });
    ...
    it('reports SFTP keys as errors by default', () => {
      ...
      const issues = checkPrerequisites();
      const sftpErrors = issues.filter((i) => i.key.startsWith('SFTP') && i.type === 'missing');
    ...
    it('treats RCON placeholder values as warnings (not errors)', () => {
      ...
      const issues = checkPrerequisites();
      const rconIssues = issues.filter((i) => i.key.startsWith('RCON'));
      assert.ok(rconIssues.every((i) => i.type === 'warning'));
    ...
    it('reports missing when neither SFTP_PASSWORD nor SFTP_PRIVATE_KEY_PATH is set', () => {
      ...
      process.env.SFTP_USER = 'valid';
      const issues = checkPrerequisites();
      const credError = issues.find((i) => i.key === 'SFTP_PASSWORD' && i.type === 'missing');
    ...
    it('accepts FTP_* fallback but warns about rename', () => {
      ...
      const sftpErrors = issues.filter(
        (i) => i.type === 'missing' && (i.key.startsWith('SFTP') || i.key.startsWith('FTP')),
      );
      assert.strictEqual(sftpErrors.length, 0, 'No missing SFTP errors when FTP_* is set');
      const ftpWarnings = issues.filter((i) => i.key.startsWith('FTP') && i.type === 'warning');
