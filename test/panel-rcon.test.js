...
function createMockPanelApi(overrides = {}) {
  return {
    available: true,
...
async function connectRcon(rcon) {
...
describe('_handleMessage', () => {
  ...
  it('auth success emits reconnect with downtime on re-connect', () => {
    ...
    const events = [];
    rcon.on('reconnect', (data) => events.push(data));
    ...
    assert.ok(events[0].downtime >= 4000, `downtime ${events[0].downtime} should be >= 4000`);
  ...
  it('console output emits output event and feeds _outputCallback', () => {
    const emitted = [];
    rcon.on('output', (line) => emitted.push(line));
    const fed = [];
    rcon._outputCallback = (line) => fed.push(line);
  ...
  it('token expiring triggers token refresh', async () => {
    ...
    rcon._ws = new MockWebSocket('wss://test');
  ...
  it('stats emits parsed JSON object', () => {
    const received = [];
    rcon.on('stats', (data) => received.push(data));
  ...
  it('status emits status string', () => {
    const statuses = [];
    rcon.on('status', (s) => statuses.push(s));
...
describe('send / _sendCommand', () => {
  it('wraps command in { event: "send command", args: [cmd] }', { timeout: 5000 }, async () => {
    ...
    const cmdMsg = rcon._ws._sends.map((s) => JSON.parse(s)).find((m) => m.event === 'send command');
...
describe('sendCached', () => {
  ...
  it('evicts stale entries when cache exceeds 50', async () => {
    const rcon = createTestRcon();
    rcon.send = async (cmd) => `result-${cmd}`;
