/* eslint-disable @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-require-imports, @typescript-eslint/no-floating-promises, @typescript-eslint/restrict-template-expressions */
...
function setupTmp(): { envPath: string; examplePath: string; backupDir: string } {
...
describe('syncEnv', () => {
  ...
  it('creates a backup of the old .env', () => {
    ...
    const backups = fs.readdirSync(backupDir).filter((f: string) => f.startsWith('.env.backup.'));
  ...
  it('prunes backups to keep only the 2 most recent', () => {
    ...
    const remaining = fs.readdirSync(backupDir).filter((f: string) => f.startsWith('.env.backup.'));
