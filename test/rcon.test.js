...
describe('_onData', () => {
  ...
  it('treats packet with oversized size field (>65536) as raw text', () => {
    ...
    assert.ok(cb.calls[0].includes('raw text data'));
  });
  ...
  it('treats packet with undersized size field (<10) as raw text', () => {
    ...
    assert.equal(cb.calls.length, 1);
    assert.ok(cb.calls[0].includes('small size'));
  });
  ...
  it('treats packet with negative size field as raw text', () => {
    ...
    assert.equal(cb.calls.length, 1);
    assert.ok(cb.calls[0].includes('negative'));
  });
...
describe('_sendPacket', () => {
  ...
  beforeEach(() => {
    rcon = createTestRcon();
    written = [];
    rcon.socket = { write: (buf) => written.push(Buffer.from(buf)) };
  });
