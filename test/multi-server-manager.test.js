/**
 * Tests for MultiServerManager — multi-server orchestration.
 * Run: node --test test/multi-server-manager.test.js
 */
...
// We only import the manager class and pure utility exports.
// ServerInstance itself requires heavy module dependencies (RCON, SFTP, etc.)
// so we test MultiServerManager's config/persistence logic through a mock ConfigRepository.
const { createServerConfig, _extractSaveName, SAVE_FILE_PATTERN } = require('../src/server/multi-server');

// ── Helpers ────────────────────────────────────────────────────────────────

/** Create a minimal serverDef for testing. */
function makeServerDef(overrides = {}) {
  return {
    id: 'srv_test1',
    name: 'Test Server 1',
    enabled: true,
...
/**
 * Build a minimal MultiServerManager-like object backed by a real
 * in-memory ConfigRepository, exposing only the persistence and
 * query methods we need to test without triggering heavy module loading.
 *
 * This avoids requiring ServerInstance (which pulls in RCON, SFTP, 20+ modules)
 * while still exercising the real DB persistence logic.
 */
function createTestManager(repo) {
  // Directly replicate the manager's core logic using the real ConfigRepository
  ...
  return {
    ...
    // ── _loadServerDefs — copied from MultiServerManager ──
    _loadServerDefs() {
      if (this._configRepo) {
        try {
    ...
    // ── addServer (simplified — no ServerInstance creation) ──
    ...
    // ── startServer (mock — just marks running) ──
    ...
    // ── stopServer (mock — just marks stopped) ──
    async stopServer(id) {
      const instance = this._instances.get(id);
...
describe('MultiServerManager', () => {
  ...
  let repo;
  /** @type {ReturnType<typeof createTestManager>} */
  let manager;
  ...
  describe('_loadServerDefs()', () => {
    ...
    it('returns server defs from configRepo', () => {
      ...
      assert.ok(defs.some((d) => d.name === 'Alpha'));
      assert.ok(defs.some((d) => d.name === 'Bravo'));
    });
  ...
  describe('getStatuses()', () => {
    it('returns status for all servers', () => {
      ...
      assert.ok(statuses.every((s) => 'running' in s));
      assert.ok(statuses.every((s) => 'modules' in s));
    });
  ...
  describe('updateServer()', () => {
    ...
    it('merges RCON updates (partial update)', async () => {
      ...
      assert.equal(updated.rcon.host, '10.0.0.1'); // preserved
      assert.equal(updated.rcon.password, 'test-rcon-not-real'); // preserved
    });
    ...
    it('merges SFTP updates (partial update)', async () => {
      ...
      assert.equal(updated.sftp.host, '10.0.0.1'); // preserved
    });
    ...
    it('merges path updates (partial)', async () => {
      ...
      assert.equal(updated.paths.logPath, '/home/game/HMZLog.log'); // preserved
    });
    ...
    it('updates channels via merge', async () => {
      ...
      assert.equal(updated.channels.serverStatus, '111'); // preserved
      assert.equal(updated.channels.chat, '222'); // preserved
    });
  });
...
// ═════════════════════════════════════════════════════════════════════════════
// Utility functions (exported from multi-server.js)
// ═════════════════════════════════════════════════════════════════════════════
