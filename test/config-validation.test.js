/**
 * Tests for config-validation.js — field-level config value validation.
 * Run: node --test test/config-validation.test.js
 */
...
describe('ENV_KEY_VALIDATORS', () => {
  ...
  it('all validator types reference existing FIELD_VALIDATORS', () => {
    for (const [key, def] of Object.entries(ENV_KEY_VALIDATORS)) {
      assert.ok(FIELD_VALIDATORS[def.type], `${key} references unknown validator type: ${def.type}`);
    }
  });
});
